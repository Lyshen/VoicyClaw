...
export function buildConversationEntries(
  ...
  return timeline
    .filter(
      (entry): entry is TimelineEntry & { role: "user" | "bot" | "preview" } =>
        entry.role !== "system",
    )
    .filter((entry) => {
...
function RoomShell({
  ...
  const dotClasses = statusTone === "live" ? "bg-emerald-300" : "bg-amber-300"
...
function CommandLine({
  ...
  const isConfig = prefix === "cfg"
  const lineClasses = isConfig
  ...
  return (
    ...
    >
      ...
      {onAction && actionLabel ? (
        <button
          className={`shrink-0 rounded-full border bg-white/[0.04] px-3 py-1 text-xs font-medium transition backdrop-blur-sm ${actionClasses}`}
          type="button"
          onClick={onAction}
        >
          {actionLabel}
